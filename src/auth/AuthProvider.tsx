...
import { UserTypeEnum, getUser as getUserQuery } from '@onextech/etc-api'
...
const AuthProvider = (props) => {
  ...
  const updateUser = async (values) => {
    const { firstName, lastName, mobile, address, country, city, state, zip } = values
  ...
  return (
    <AuthContext.Provider value={auth}>
      <>
        ...
        >
          <Alert severity="success" className={classes.loginSuccess} onClose={handleClose}>
            Login Success
          </Alert>
        </Snackbar>
        {children}
